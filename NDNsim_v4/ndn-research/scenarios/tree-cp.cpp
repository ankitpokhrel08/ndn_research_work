...
int main(int argc, char* argv[])
{
  ...
  double   attackRate   = 100.0;  // per-attacker interest rate (interests/s)
  ...
  tag << "tree-cp-r" << (int)attackRate << "-run" << rngRun;
  ...
  consumerHelper.SetAttribute("NumberOfContents", StringValue("300"));
  ...
  // CP attack: 50 junk prefixes per attacker, frequency scaled to attackRate
  // 50 names × cpFreq = attackRate per attacker
  double cpFreq = attackRate / 50.0;

  for (int i = 0; i < 50; i++) {
    ns3::ndn::AppHelper atk1("ns3::ndn::ConsumerCbr");
    atk1.SetAttribute("Prefix", StringValue("/ndn/junk/a" + std::to_string(i)));
    atk1.SetAttribute("Frequency", StringValue(std::to_string(cpFreq)));
    ...
    atk2.SetAttribute("Frequency", StringValue(std::to_string(cpFreq)));
    auto a2 = atk2.Install(c6);
    a2.Start(Seconds(attackStart));
  }

  // Tracers with parameterized filenames
