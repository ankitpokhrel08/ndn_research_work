...
int main(int argc, char* argv[])
{
  ...
  // Seed control — different rngRun = different stochastic realization
  ns3::RngSeedManager::SetSeed(1);
  ns3::RngSeedManager::SetRun(rngRun);

  system("mkdir -p results");

  // Build output tag: tree-ifa-r30-run4
  std::ostringstream tag;
  tag << "tree-ifa-r" << (int)attackRate << "-run" << rngRun;
  ...
  // BlackholeProducer for /evil — interests route here and time out
  ...
  // Legitimate consumers — Zipf so baseline matches normal and CP scenarios
  ...
  // IFA: 4 streams from c1, 40:30:20:10 ratio scaled to attackRate
