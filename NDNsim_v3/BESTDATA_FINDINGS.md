...
> **Version v3 — catalog-fixed sweep (177 runs). Superseded by
> [`../NDNsim_v4/Finalfindings.md`](../NDNsim_v4/Finalfindings.md).**
> This is where the data became honest: `NumberOfContents` 100 → **300** revived the
> background traffic (alive for the full 600 s), so the floor is measured against a *busy*
> network for the first time. It established the *shape* of both floors and a real ~1–2%
> FPR — but the CP 90% crossing was only **interpolated** across an unsampled r30→r50 gap,
> and IFA looked **flat at 100%** because the sweep stopped at 10 Hz. The v4 top-up
> (CP {35,40,45}, IFA {1,2,4,6,8}, 5 seeds each) closed both gaps: **measured floors
> CP ≈ 44 Hz, IFA ≈ 1.8 Hz** — IFA does have a floor, ~25× lower than CP's. Kept as the
> record of this stage (lineage: v1 single-rate → v2 first sweep, broken →
> **v3 catalog-fixed** → v4 final).
...
- **The CP 90% crossing is interpolated here** (between r30 ~60% and r50 100%, with r50/r100
  single-seed). **Resolved in v4** ([`../NDNsim_v4/Finalfindings.md`](../NDNsim_v4/Finalfindings.md)):
  the r35/40/45 top-up runs measured the crossing directly (71 / 83 / 92%) → floor ≈ 44 Hz.
  IFA, which looks flat 100% here because this sweep stopped at 10 Hz, was probed down to 1 Hz
  in v4 and has a floor ≈ 1.8 Hz.
