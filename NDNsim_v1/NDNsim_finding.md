...
> **Version v1 — single-rate study (9 runs: 3 topologies × {normal, IFA, CP}).**
> This is the *original* NDNsim result and the home of the deep per-node analysis:
> spatial localization, the feature ablation, and the KS discriminability study. Its
> headline numbers (100% attacker detection, ~1 s latency, 0% held-out FPR) are correct
> **for a single high attacker rate (~100 Hz)**, but carry two limits the later versions
> exist to remove:
> 1. **One rate only** — so it cannot show a *detection floor* (how low the attacker can go
>    before the detector misses it). The rate sweep that answers this is v2→v4.
> 2. **Dead background traffic** — every scenario used `ConsumerZipfMandelbrot` with
>    `NumberOfContents = 100`, which exhausts the catalog ~10 s in, so the legitimate
>    consumers fall silent long before the attack window. CP's "100% / 0% FPR" was therefore
>    partly measured *against a quiet network*. Raising the catalog to 300 (v3+) keeps the
>    background alive for the full run.
>
> **Use this doc for:** the localization / ablation / KS analysis (still valid — those are
> structural properties of the attacks). **For the detection floor and the corrected,
> live-traffic numbers, see [`../NDNsim_v4/Finalfindings.md`](../NDNsim_v4/Finalfindings.md)**
